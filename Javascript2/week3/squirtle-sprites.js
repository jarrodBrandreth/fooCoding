...
const squirtle = JSON.parse(fetchPokemonData());
...
const dance = () => {
  ...
  container.setAttribute('id', 'background-change');
  back.forEach((img) => img.classList.add('back-dance'));
  front.forEach((img) => img.classList.add('front-dance'));

  const removeClass = () => {
    container.removeAttribute('id', 'background-change');
    back.forEach((img) => img.classList.remove('back-dance'));
    front.forEach((img) => img.classList.remove('front-dance'));
  };

  setTimeout(removeClass, 6100);
};

const showSquirtles = (obj) => {
  const container = document.createElement('div');
  container.classList.add('container');

  const defaultSquirtle = document.createElement('div');
  defaultSquirtle.classList.add('default');

  const shinySquirtle = document.createElement('div');
  shinySquirtle.classList.add('shiny');

  const danceBtn = document.createElement('button');
  danceBtn.innerText = 'DANCE PARTY!';
  danceBtn.setAttribute('id', 'dance-btn');
  danceBtn.addEventListener('click', dance);

  const arr = Object.entries(obj.sprites)
    .filter((item) => item[1] !== null)
    .forEach((item) => {
      const img = document.createElement('img');
      img.setAttribute('src', `${item[1]}`);

      if (item[0].includes('back')) {
        img.classList.add('back');
      } else {
        img.classList.add('front');
      }

      if (item[0].includes('default')) {
        defaultSquirtle.append(img);
      } else {
        shinySquirtle.append(img);
      }
    });
  container.append(defaultSquirtle, shinySquirtle, danceBtn);
  body.prepend(container);
};

showSquirtles(squirtle);
